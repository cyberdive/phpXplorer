...
class pxCLS_action_edit_create_zip extends pxCLS_action_edit_create{

	var $sNewName;
	var $sSelection;
	var $oArchive;

	function pxCLS_action_edit_create_zip(){
	...
	function addFilesetToZipArchive($sDir, $aFileset){
		...
		foreach($aFileset as $oFile){
			if(!$oFile->bDirectory){
				$this->oArchive->addFile($sDir . "/" . $oFile->sFile, str_replace($pxp->sWorkingDirectory . "/", "", $sDir . "/" . $oFile->sFile));
			}else{
				$this->addFilesetToZipArchive($sDir . "/" . $oFile->sFile, $oFile->aFileset);
			}
		}
	}
	...
	function handleRequest(){
  	...
  	if($this->sRequestAction == "download"){

			if($this->sNewName == "")
				$this->sNewName = "archive." . $this->sType;

     	$aFileset = $pxp->getFileset($pxp->sWorkingDirectory, explode(",", $this->sSelection), $pxp->aData, true);

     	$this->addFilesetToZipArchive($pxp->sWorkingDirectory, $aFileset);
		
			header("Content-type: application/zip");
	 		header("Content-Disposition: attachment; filename=" . $this->sNewName);

			echo $this->oArchive->file();
      	...
      	$this->addFilesetToZipArchive($pxp->sWorkingDirectory, $aFileset);
				...
				$this->oArchive->writeFile($pxp->sWorkingDirectory . "/" . $this->sNewName);
  		}
  	}
	...
	function getBodyHTML(){
		...
		$sHTML = '<table cellspacing="0" cellpadding="10">'
						...
						. '<button name="btnDownload" onclick="send(\'download\')" class="action">' . $pxp->aLanguages[$pxp->oUser->sLanguage]['download'] . '</button>'
